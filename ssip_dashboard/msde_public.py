...
"""Governed Ministry of Skill Development & Entrepreneurship public bundle.

MSDE records are loaded from a hash-verified publication snapshot.  The public
page deliberately keeps permanent scheme identities, dated application calls,
and historical references in separate views; a scheme page alone never implies
...
PUBLICATION_DIR = Path("data/departments/msde/v3_4_11_0")
MANIFEST_NAME = "active_publication_manifest_v3_4_11_0.json"
OFFICIAL_HOSTS = (
    "msde.gov.in", "skillindiadigital.gov.in", "nsdcindia.org", "dgt.gov.in",
    "ncvet.gov.in", "niesbud.nic.in", "pib.gov.in",
)
...
class MSDESupplementError(RuntimeError):
    """Raised when the active MSDE publication gate fails."""


@dataclass(frozen=True)
class MSDESupplement:
    records: tuple[dict[str, Any], ...]
    manifest: dict[str, Any]


@dataclass(frozen=True)
class MSDEPublicBundle:
...
def is_official_msde_url(value: str) -> bool:
    try:
...
def _record_from_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    if not is_official_msde_url(source_url):
        raise MSDESupplementError(f"Unsafe MSDE source URL: {source_url}")
    application_url = str(row.get("application_url") or "").strip()
    if application_url and not is_official_msde_url(application_url):
        raise MSDESupplementError(f"Unsafe MSDE application URL: {application_url}")
    ...
    return {
        ...
        "source": row.get("source", "Ministry of Skill Development & Entrepreneurship").strip(),
        "ministry": row.get("ministry", "Ministry of Skill Development & Entrepreneurship").strip(),
        "department": row.get("department", "Ministry of Skill Development & Entrepreneurship (MSDE)").strip(),
        "implementing_agency": row.get("implementing_agency", "MSDE").strip(),
        ...
        "is_public": 1,
        "current_location": "MSDE_ACTIVE_PUBLICATION",
...
def load_active_msde_supplement(project_root: Path) -> MSDESupplement:
    directory = (project_root / PUBLICATION_DIR).resolve()
    manifest_path = directory / MANIFEST_NAME
    if not manifest_path.exists():
        return MSDESupplement(records=(), manifest={"status": "NOT_CONFIGURED"})
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise MSDESupplementError(f"Cannot read active MSDE manifest: {manifest_path}") from exc
    if manifest.get("activation_status") != "ACTIVE":
        return MSDESupplement(records=(), manifest=manifest)
    inventory_path = (directory / str(manifest.get("inventory_file", "")).strip()).resolve()
    if directory not in inventory_path.parents:
        raise MSDESupplementError("Active MSDE inventory path escapes the governed directory.")
    try:
        payload = inventory_path.read_bytes()
    except OSError as exc:
        raise MSDESupplementError(f"Cannot read active MSDE inventory: {inventory_path}") from exc
    if sha256(payload).hexdigest() != manifest.get("inventory_sha256"):
        raise MSDESupplementError("Active MSDE inventory hash does not match its manifest.")
    ...
    if not ids or not all(ids) or len(ids) != len(set(ids)):
        raise MSDESupplementError("Duplicate or blank master IDs in active MSDE inventory.")
    if len(records) != int(manifest.get("record_count", -1)):
        raise MSDESupplementError("Active MSDE record count does not match its manifest.")
    return MSDESupplement(records=records, manifest=manifest)


def build_msde_public_bundle(records: Iterable[CatalogueRecord]) -> MSDEPublicBundle:
    owned = [
        record for record in records
        if record.current_location == "MSDE_ACTIVE_PUBLICATION"
        or "skill development and entrepreneurship" in " ".join((record.department, record.source)).casefold()
    ]
    ...
    for record in owned:
        ...
        documents.extend(
            {"title": record.scheme_name, "document_type": "GUIDELINE", "official_url": url,
             "department_label": "Ministry of Skill Development & Entrepreneurship (MSDE)"}
            for url in record.guideline_urls if is_official_msde_url(url)
        )
...
def filter_msde_records(records: Iterable[CatalogueRecord], *, keyword: str = "", record_type: str = "All", status: str = "All") -> list[CatalogueRecord]:
