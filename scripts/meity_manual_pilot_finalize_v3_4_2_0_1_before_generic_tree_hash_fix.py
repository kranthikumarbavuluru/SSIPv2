...
def tree_hash(root: Path) -> str:
    if not root.exists():
        return "MISSING"
    root = root.resolve()
    files = sorted((p for p in root.rglob("*") if p.is_file()), key=lambda p: str(p.resolve()).lower())
    lines = [f"{str(p.resolve().relative_to(root))}\t{sha256_file(p)}" for p in files]
    payload = ((os.linesep.join(lines) + os.linesep).encode("utf-8") if lines else b"")
