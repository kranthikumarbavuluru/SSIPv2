...
"""Governed Innovations for Defence Excellence (iDEX) public bundle.

The dashboard consumes a hash-verified iDEX snapshot.  It keeps the permanent
iDEX/ADITI identities separate from dated challenge windows and never treats a
challenge as open without dated evidence from the official iDEX portal.
"""
...
PUBLICATION_DIR = Path("data/departments/idex/v3_4_9_0")
MANIFEST_NAME = "active_publication_manifest_v3_4_9_0.json"
OFFICIAL_HOSTS = ("idex.gov.in", "ddpmod.gov.in", "mod.gov.in")
...
class IDEXSupplementError(RuntimeError):
    """Raised when the active iDEX publication gate fails."""


@dataclass(frozen=True)
class IDEXSupplement:
    records: tuple[dict[str, Any], ...]
    manifest: dict[str, Any]


@dataclass(frozen=True)
class IDEXPublicBundle:
...
def is_official_idex_url(value: str) -> bool:
    try:
...
def _record_from_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    if not is_official_idex_url(source_url):
        raise IDEXSupplementError(f"Unsafe iDEX source URL: {source_url}")
    application_url = row.get("application_url", "").strip()
    if application_url and not is_official_idex_url(application_url):
        raise IDEXSupplementError(f"Unsafe iDEX application URL: {application_url}")
    ...
    return {
        ...
        "source": row.get("source", "iDEX / Defence Innovation Organisation").strip(),
        "ministry": row.get("ministry", "Ministry of Defence").strip(),
        "department": row.get("department", "Department of Defence Production").strip(),
        "implementing_agency": row.get("implementing_agency", "Defence Innovation Organisation (DIO)").strip(),
        ...
        "is_public": 1,
        "current_location": "IDEX_ACTIVE_PUBLICATION",
...
def load_active_idex_supplement(project_root: Path) -> IDEXSupplement:
    directory = (project_root / PUBLICATION_DIR).resolve()
    manifest_path = directory / MANIFEST_NAME
    if not manifest_path.exists():
        return IDEXSupplement(records=(), manifest={"status": "NOT_CONFIGURED"})
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IDEXSupplementError(f"Cannot read active iDEX manifest: {manifest_path}") from exc
    if manifest.get("activation_status") != "ACTIVE":
        return IDEXSupplement(records=(), manifest=manifest)
    ...
    if directory not in inventory_path.parents:
        raise IDEXSupplementError("Active iDEX inventory path escapes the governed directory.")
    try:
        payload = inventory_path.read_bytes()
    except OSError as exc:
        raise IDEXSupplementError(f"Cannot read active iDEX inventory: {inventory_path}") from exc
    if sha256(payload).hexdigest() != manifest.get("inventory_sha256"):
        raise IDEXSupplementError("Active iDEX inventory hash does not match its manifest.")
    ...
    if not ids or not all(ids) or len(ids) != len(set(ids)):
        raise IDEXSupplementError("Duplicate or blank master IDs in active iDEX inventory.")
    if len(records) != int(manifest.get("record_count", -1)):
        raise IDEXSupplementError("Active iDEX record count does not match its manifest.")
    return IDEXSupplement(records=records, manifest=manifest)


def build_idex_public_bundle(records: Iterable[CatalogueRecord]) -> IDEXPublicBundle:
    owned = [
        record for record in records
        if record.current_location == "IDEX_ACTIVE_PUBLICATION"
        or "defence innovation organisation" in " ".join((record.department, record.source, record.implementing_agency)).casefold()
    ]
        ...
        if record.guideline_urls:
            documents.extend(
                {"title": record.scheme_name, "document_type": "GUIDELINE", "official_url": url,
                 "department_label": "Department of Defence Production / iDEX"}
                for url in record.guideline_urls if is_official_idex_url(url)
            )
...
def filter_idex_records(records: Iterable[CatalogueRecord], *, keyword: str = "", record_type: str = "All", status: str = "All") -> list[CatalogueRecord]:
