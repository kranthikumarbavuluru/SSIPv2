...
def build_evidence(row: dict[str, str], project_root: Path) -> tuple[str, list[str]]:
    """Build an independent evidence corpus.

    Candidate fields being validated (programme_status, deadline, funding values,
    eligibility, benefits, and LLM evidence notes) are intentionally excluded.
    Otherwise the validator would merely confirm that an extracted value appears
    ...
    if page_title:
        ...
        sources.append("FIELD:page_title")
...
def numeric_supported(value: Any, evidence_text: str) -> bool:
    """Require explicit monetary context around a candidate funding amount."""
    text = clean(value)
    if not text:
        return True
    try:
    ...
    # A bare four-digit year is not a funding amount.
    ...
    evidence = clean(evidence_text)
    money_context = re.compile(
        r"(?:₹|rs\.?|inr|rupees?|usd|dollars?|lakh|lakhs|crore|crores|"
        r"million|billion|grant|funding|financial assistance|amount|support)",
        re.IGNORECASE,
    )
    ...
    for variant in sorted(variants, key=len, reverse=True):
        if not variant:
            continue
        for match in re.finditer(re.escape(variant), evidence, re.IGNORECASE):
            start = max(0, match.start() - 80)
            end = min(len(evidence), match.end() + 80)
            if money_context.search(evidence[start:end]):
                return True
...
def status_supported(status: str, evidence_text: str, deadline: str, as_of: date) -> bool:
    ...
    # Historical evidence is a provenance state, not a publishable current status.
    if normalized == "HISTORICAL_EVIDENCE_ONLY":
        return False
...
def classify_negative_page(row: dict[str, str], evidence_text: str) -> list[str]:
    # Page-type rejection must be dominated by the page title/URL, not by a
    # phrase that happens to occur somewhere inside a long guideline document.
    title_url_text = " ".join(
        [
    ...
    for code, phrases in NEGATIVE_PAGE_PATTERNS.items():
        if any(phrase in title_url_text for phrase in phrases):
            hits.append(code)
    return hits


def programme_identity_strength(row: dict[str, str], evidence_text: str) -> float:
    # Upstream identity score is only a prior; independent page evidence must
    # carry most of the final identity score.
    score = clamp(parse_float(row.get("page_identity_score"), 0.0)) * 0.40
    ...
    score += min(0.18, keyword_count * 0.03)
    return clamp(score)
...
def evaluate_record(
    ...
    master_type = clean(row.get("master_type")).upper()
    historical_only = (
        programme_status == "HISTORICAL_EVIDENCE_ONLY"
        or "HISTORICAL_EVIDENCE" in master_type
    )
    final_url_lower = final_url.casefold()
    historical_call_page = historical_only and (
        "callforproposals" in final_url_lower
        or "call-for-proposal" in final_url_lower
        or final_url_lower.endswith(".pdf")
    )
    parsed_deadline = parse_date(deadline)
    deadline_passed = bool(parsed_deadline and parsed_deadline < as_of)
    funding_year_contamination = False
    deadline_year = parsed_deadline.year if parsed_deadline else None
    for candidate in (funding_min, funding_max):
        try:
            candidate_number = float(candidate.replace(",", "")) if candidate else None
        except ValueError:
            candidate_number = None
        if candidate_number is not None and candidate_number.is_integer():
            candidate_year = int(candidate_number)
            if 1900 <= candidate_year <= 2100 and (
                deadline_year is None or candidate_year == deadline_year
            ):
                funding_year_contamination = True

    normalized_deadline = deadline if deadline_ok else ""
    normalized_funding_min = funding_min if funding_min_ok and not funding_year_contamination else ""
    normalized_funding_max = funding_max if funding_max_ok and not funding_year_contamination else ""
    ...
    if historical_only:
        append_unique(warnings, "HISTORICAL_EVIDENCE_NOT_CURRENT_PROGRAMME_STATUS")
        append_unique(actions, "Discover a current official programme page before database approval.")
    if historical_call_page:
        append_unique(warnings, "HISTORICAL_CALL_PAGE_NOT_CORE_PROGRAMME_PAGE")
    if deadline_passed:
        append_unique(warnings, "DEADLINE_PASSED_AS_OF_VALIDATION_DATE")
    if funding_year_contamination:
        append_unique(warnings, "FUNDING_VALUE_APPEARS_TO_BE_YEAR")
        append_unique(actions, "Remove the year-like funding value and re-extract monetary evidence.")
    ...
    if historical_only:
        confidence_after = min(confidence_after, 0.55)
    if not status_ok:
        confidence_after = min(confidence_after, 0.69)
    ...
    unsupported_important_values = funding_year_contamination or any(
        item in warnings
        for item in (
    ...
    if unsupported_important_values:
        confidence_after = min(confidence_after, 0.60)
    ...
    elif historical_only or historical_call_page:
        decision = "NEEDS_MORE_EVIDENCE"
        append_unique(reasons, "HISTORICAL_EVIDENCE_REQUIRES_CURRENT_CORE_PROGRAMME_PAGE")
    elif evidence_missing or identity_strength < 0.60:
        decision = "NEEDS_MORE_EVIDENCE"
        append_unique(reasons, "INSUFFICIENT_INDEPENDENT_EVIDENCE")
    elif not status_ok or unsupported_important_values or negative_hits:
        decision = "NEEDS_ADMIN_REVIEW"
    ...
    elif official and application_official and identity_strength >= 0.68 and status_ok and not unsupported_important_values and not negative_hits and confidence_after >= 0.72:
    ...
    elif identity_strength >= 0.60 and official:
        decision = "NEEDS_ADMIN_REVIEW"
        append_unique(reasons, "GENUINE_PROGRAMME_WITH_AMBIGUOUS_OR_CONFLICTING_FIELDS")
    else:
        decision = "NEEDS_MORE_EVIDENCE"
    ...
    output.update(
        {
            ...
            "historical_evidence_only": "YES" if historical_only else "NO",
            "historical_call_page": "YES" if historical_call_page else "NO",
            "deadline_passed": "YES" if deadline_passed else "NO",
            "funding_year_contamination": "YES" if funding_year_contamination else "NO",
...
def run_validation(
    ...
    subsets = {
        "approved_for_database_v2_7_2_1.csv": [row for row in validated_rows if row["validation_decision"] == "APPROVED_FOR_DATABASE"],
        "admin_review_queue_v2_7_2_1.csv": [row for row in validated_rows if row["validation_decision"] == "NEEDS_ADMIN_REVIEW"],
        "needs_more_evidence_v2_7_2_1.csv": [row for row in validated_rows if row["validation_decision"] == "NEEDS_MORE_EVIDENCE"],
        "rejected_records_v2_7_2_1.csv": [row for row in validated_rows if row["validation_decision"] == "REJECTED"],
    }
    ...
    output_paths = {
        "validated": output_directory / "validated_records_v2_7_2_1.csv",
        "audit": output_directory / "validation_audit_v2_7_2_1.csv",
        "handoff": output_directory / "database_handoff_v2_7_2_1.csv",
    }
    ...
    summary_path = output_directory / "validation_summary_v2_7_2_1.json"
...
def print_summary(summary: dict[str, Any]) -> None:
    print("=" * 78)
    print("SSIP v2.7.2.1 STRICT VALIDATION HOTFIX AND DATABASE HANDOFF — DRY RUN")
    print("=" * 78)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="SSIP v2.7.2.1 deterministic strict validation hotfix dry run")
    ...
    parser.add_argument("--output-directory", default="data/incremental/v2_7_2_1_strict_validation_hotfix")
    ...
    try:
        ...
        print(f"\nSummary written to:\n{output_directory / 'validation_summary_v2_7_2_1.json'}")
        return 0
