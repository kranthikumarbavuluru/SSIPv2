...
"""Governed agriculture-startup and agri-innovation public bundle."""
...
PUBLICATION_DIR = Path("data/departments/agri_startup/v3_4_10_0")
MANIFEST_NAME = "active_publication_manifest_v3_4_10_0.json"
OFFICIAL_HOSTS = (
    "agriwelfare.gov.in", "pib.gov.in", "nabard.org", "sfacindia.com",
    "manage.gov.in", "icar.gov.in", "apeda.gov.in", "angraurabitpt.org", "angrau.ac.in", "pdkvrif.com", "afbic.iitkgp.ac.in", "abif.iitkgp.ac.in", "rkvy-raftaariitbhu.org", "rkvy.da.gov.in",
    "agriinfra.dac.gov.in", "pmkisan.gov.in", "agristartup.gov.in",
)
...
class AgriStartupSupplementError(RuntimeError):
    """Raised when the active agriculture-startup publication gate fails."""


@dataclass(frozen=True)
class AgriStartupSupplement:
    records: tuple[dict[str, Any], ...]
    manifest: dict[str, Any]


@dataclass(frozen=True)
class AgriStartupPublicBundle:
...
def is_official_agri_startup_url(value: str) -> bool:
    try:
...
def _record_from_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    if not is_official_agri_startup_url(source_url):
        raise AgriStartupSupplementError(f"Unsafe agriculture-startup source URL: {source_url}")
    ...
    return {
        ...
        "source": row.get("source", "Agriculture & Farmers Welfare startup ecosystem").strip(),
        "ministry": row.get("ministry", "Ministry of Agriculture & Farmers Welfare").strip(),
        "department": row.get("department", "Department of Agriculture & Farmers Welfare").strip(),
        "implementing_agency": row.get("implementing_agency", "Department of Agriculture & Farmers Welfare").strip(),
        "parent_master_id": row.get("parent_master_id", "").strip(),
        "parent_scheme_name": row.get("parent_scheme_name", "").strip(),
        "applicant_layer": "DIRECT_SUPPORT",
        ...
        "is_public": 1,
        "current_location": "AGRI_STARTUP_ACTIVE_PUBLICATION",
        ...
        "funding_amount_status": "RECORDED" if row.get("funding_maximum", "").strip() else "NOT_STATED",
        ...
        "search_blob": " ".join(
            value.strip()
            for value in (row.get("scheme_code", ""), row.get("canonical_name", ""), row.get("objectives", ""), row.get("benefits", ""), row.get("sectors", ""), row.get("scheme_types", ""))
            if value.strip()
        ).casefold(),
    }


def load_active_agri_startup_supplement(project_root: Path) -> AgriStartupSupplement:
    directory = (project_root / PUBLICATION_DIR).resolve()
    manifest_path = directory / MANIFEST_NAME
    if not manifest_path.exists():
        return AgriStartupSupplement(records=(), manifest={"status": "NOT_CONFIGURED"})
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise AgriStartupSupplementError(f"Cannot read active agriculture-startup manifest: {manifest_path}") from exc
    if manifest.get("activation_status") != "ACTIVE":
        return AgriStartupSupplement(records=(), manifest=manifest)
    inventory_path = (directory / str(manifest.get("inventory_file", "")).strip()).resolve()
    if directory not in inventory_path.parents:
        raise AgriStartupSupplementError("Active agriculture-startup inventory path escapes its governed directory.")
    try:
        payload = inventory_path.read_bytes()
    except OSError as exc:
        raise AgriStartupSupplementError(f"Cannot read active agriculture-startup inventory: {inventory_path}") from exc
    if sha256(payload).hexdigest() != manifest.get("inventory_sha256"):
        raise AgriStartupSupplementError("Active agriculture-startup inventory hash does not match its manifest.")
    ...
    if not ids or not all(ids) or len(ids) != len(set(ids)):
        raise AgriStartupSupplementError("Duplicate or blank master IDs in active agriculture-startup inventory.")
    if len(records) != int(manifest.get("record_count", -1)):
        raise AgriStartupSupplementError("Active agriculture-startup record count does not match its manifest.")
    return AgriStartupSupplement(records=records, manifest=manifest)


def build_agri_startup_public_bundle(records: Iterable[CatalogueRecord]) -> AgriStartupPublicBundle:
    owned = [record for record in records if record.current_location == "AGRI_STARTUP_ACTIVE_PUBLICATION"]
    ...
    for record in owned:
        ...
        documents.extend(
            {"title": record.scheme_name, "document_type": "GUIDELINE", "official_url": url, "department_label": "Agriculture & Farmers Welfare startup ecosystem"}
            for url in record.guideline_urls if is_official_agri_startup_url(url)
        )
    ...
    return AgriStartupPublicBundle(tuple(permanent), tuple(current), tuple(historical), tuple(documents), excluded, max((item for item in dates if item), default="Not recorded"))


def filter_agri_startup_records(records: Iterable[CatalogueRecord], *, keyword: str = "", record_type: str = "All", status: str = "All") -> list[CatalogueRecord]:
