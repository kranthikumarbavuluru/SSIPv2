...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="ssip_meity_validation_v2_4_") as temp_dir:
        root = Path(temp_dir)
        data_dir = root / "data"
        ...
        write_json(data_dir / "approved_for_database_v1.json", existing_approved)
        ...
        assert result.summary["hotfix_version"] == "2.4.0"
        ...
        canonical_approved = json.loads(
            (data_dir / "approved_for_database_v1.json").read_text(encoding="utf-8")
        )
        ...
        assert len(canonical_approved) == 4
        ...
        print("MeitY Incremental Validation v2.4 self-test passed.")
